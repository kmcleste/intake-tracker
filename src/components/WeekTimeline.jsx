...
export default function WeekTimeline({ entries }) {
  ...
  return (
    <div>
      ...
      >
        <span style={{ fontFamily: "var(--c-font-mono)", fontSize: 9, color: "var(--c-text-subtle)", letterSpacing: "0.12em" }}>
          7-DAY OVERVIEW
        </span>
        <span style={{ fontFamily: "var(--c-font-mono)", fontSize: 9, color: "var(--c-text-subtle)", marginLeft: "auto", opacity: 0.5 }}>
          {open ? "▴" : "▾"}
        </span>
      </div>
      ...
      {open && (
        <>
          <div style={{ display: "flex", gap: 8, paddingBottom: 6, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
              <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#c97a7a" }} />
              <span style={{ fontFamily: "var(--c-font-mono)", fontSize: 8, color: "var(--c-text-subtle)" }}>symptoms</span>
            </div>
          </div>

          <div style={{ display: "flex", gap: 4, padding: "4px 0 10px", overflowX: "auto" }}>
            {days.map(day => {
              ...
              const avgWellbeing = wellbeingVals.length
                ? Math.round(wellbeingVals.reduce((a, b) => a + b, 0) / wellbeingVals.length)
                : null;
              const symptomCount = dayEntries.reduce((n, e) => n + (e.symptoms?.length ?? 0), 0);

              const dotColor = !avgWellbeing
                ? "var(--c-border)"
                : avgWellbeing <= 2 ? "#c97a7a"
                : avgWellbeing === 3 ? "var(--c-text-warm)"
                : "var(--c-accent-mid)";
